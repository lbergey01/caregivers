...
if (!function_exists('email_body')) {
  function email_body(string $template, array $options = [], bool $security_override = false): string
  {
    ...
    // 2) Forbid absolute paths and obvious badness
    if ($template === '' || $template[0] === '/' || str_contains($template, "\0")) {
      return '';
    }

    // 3) Enforce extension policy: only .php
    if (!str_ends_with($template, '.php')) {
      $template .= '.php';
    }

    // 4) Strict allowlist: subfolders allowed, but only sane characters
    // - segments separated by /
    // - no empty segments
    // - final must end with .php
    if (!preg_match('#^[A-Za-z0-9_-]+(?:/[A-Za-z0-9_-]+)*\.php$#', $template)) {
      return '';
    }

    // 5) Hard block traversal even if someone tries to get clever
    // (regex already prevents dots, but keep this as a second belt)
    ...
    // Base directories (filesystem)
    $baseCustom = $abs_us_root . $us_url_root . 'usersc/views/';
    $baseCore   = $abs_us_root . $us_url_root . 'users/views/';

    // Resolve base dirs
    $baseCustomReal = realpath($baseCustom);
    $baseCoreReal   = realpath($baseCore);

    if ($baseCustomReal === false && $baseCoreReal === false) {
      return '';
    }
    ...
    $resolveInside = static function ($baseReal, string $base, string $rel) {
      ...
      // Ensure baseReal ends with directory separator
      $baseRealNorm = rtrim($baseReal, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR;

      // Prefix check guarantees candidate is inside base
      if (strncmp($candidateReal, $baseRealNorm, strlen($baseRealNorm)) !== 0) {
        return false;
      }
    ...
    $pathCustomReal = $resolveInside($baseCustomReal, $baseCustom, $template);
    $pathCoreReal   = $resolveInside($baseCoreReal,   $baseCore,   $template);
    ...
    if ($pathCustomReal !== false) {
      require $pathCustomReal;
    } elseif ($pathCoreReal !== false) {
      require $pathCoreReal;
    } else {
      // error_log("email_body template not found or blocked: $template");
    }
